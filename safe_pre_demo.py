...
if __name__ == '__main__':
    ...
    rgb_file = "/home/vector/Tan/xunfeidan/Metric3D/training/data/WildUAV/mapping_set_4xr/seq00/img/000025.jpg"
    ...
    # pred_conf = output_dict['safe_prediction'][1].squeeze()
    # pred_conf = pred_conf[pad_info[0]: pred_conf.shape[0] - pad_info[1],
    #             pad_info[2]: pred_conf.shape[1] - pad_info[3]]
    # pred_conf = F.interpolate(pred_conf[None, None, :, :], rgb_origin.shape[:2], mode='bilinear').squeeze()
    # pred_conf = F.sigmoid(pred_conf)
    #
    ...
    # probs = torch.sigmoid(safe_pre_logits)
    # pred_labels = (probs>0.5).float()
    # pred_labels = pred_labels.cpu().numpy()
    # pred_labels = cv2.resize(pred_labels, (rgb_origin.shape[1], rgb_origin.shape[0]), interpolation=cv2.INTER_NEAREST)

    # conf_mask = pred_conf > 0.3
    # pred_labels_mask = pred_labels == 1
    #
    # unsafe_mask = pred_labels == 0
    # unsafe_conf = pred_conf[unsafe_mask]
    # unsafe_conf_max = np.max(unsafe_conf)
    # unsafe_conf_min = np.min(unsafe_conf)
    # unsafe_conf_median = np.median(unsafe_conf)
    #
    # safe_conf = pred_conf[pred_labels_mask]
    # safe_conf_max = np.max(safe_conf)
    # safe_conf_min = np.min(safe_conf)
    # safe_conf_median = np.median(safe_conf)
    safe_gt = rgb_file.replace('img', 'safe_ann')
    safe_gt = safe_gt.replace('jpg', 'png')
    ...
    # 第一个子图：二进制掩膜
    ax1.imshow(pred_labels, cmap=cmap, vmin=0, vmax=1)
    ax1.set_title('pred seg')
    ax1.axis('off')

    # 第二个子图：二进制掩膜2
    ax2.imshow(pred_labels_2, cmap=cmap, vmin=0, vmax=1)
    ax2.set_title('pred seg 2')
    ax2.axis('off')

    # 第三个子图：原图
    ax3.imshow(cv2.cvtColor(rgb_origin, cv2.COLOR_BGR2RGB))
    ax3.set_title('Raw Image')
    ax3.axis('off')

    # 第四个子图：真实分割
    ax4.imshow(gt, cmap=cmap, vmin=0, vmax=1)
    ax4.set_title('Ground Truth Segmentation')
    ax4.axis('off')

    # 第五个子图：绘制pred_labels中等于0的部分
    highlight = np.zeros_like(rgb_origin)
    highlight[pred_labels == 0] = [0, 255, 0]  # 红色
    overlay_image = cv2.addWeighted(rgb_origin, 1, highlight, 0.5, 0)
    ax5.imshow(cv2.cvtColor(overlay_image, cv2.COLOR_BGR2RGB))
    ax5.set_title('Highlighted Areas (pred_labels == 0)')
    ax5.axis('off')

    # 第六个子图：绘制pred_labels中等于0的部分的轮廓
    # 找到轮廓
    contours, _ = cv2.findContours(pred_labels.astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    filtered_contours = [cnt for cnt in contours if cv2.contourArea(cnt) > 50]  # 过滤小轮廓
    contour_image = np.zeros_like(rgb_origin)
    cv2.drawContours(contour_image, filtered_contours, -1, (0, 255, 0), thickness=cv2.FILLED)  # 绿色填充轮廓
    overlay_contours = cv2.addWeighted(rgb_origin, 1, contour_image, 0.5, 0)
    ax6.imshow(cv2.cvtColor(overlay_contours, cv2.COLOR_BGR2RGB))
    ax6.set_title('Filled Contours of Areas (pred_labels == 0)')
    ax6.axis('off')
    ...
    # ax2.axis('off')
    #
    # # filter_mask = pred_labels_mask & conf_mask
    # # pred_labels[filter_mask] = 1
    # # pred_labels[~filter_mask] = 0
    # # pred_labels = (probs > 0.4).float()
    # # pred_labels = pred_labels.cpu().numpy()
    # ax3.imshow(rgb_origin)
    # ax3.set_title('raw img')
    # ax3.axis('off')
    #
    # ax4.imshow(gt)
    # ax4.set_title('Ground Truth Segmentation')
