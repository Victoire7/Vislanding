...
if mode == "labled":
    ...
    for sp in split:
        ...
        for target in tqdm.tqdm(file_list):
            ...
            safe_path = os.path.join(sub_root, seq_name, 'safe_ann', img_name.replace('jpg','png'))
            ...
            if os.path.exists(safe_path):
                info_dict['safe_ann'] = safe_path
            files_.append(info_dict)
        save_path = os.path.join(save_root, f'wild_{mode}_{sp}_4xr_safe.json')
    ...
    paras = [fx, fy, cx, cy]
    files_ = []
    sub_root = os.path.join(common_root, 'video_set_4xr')
