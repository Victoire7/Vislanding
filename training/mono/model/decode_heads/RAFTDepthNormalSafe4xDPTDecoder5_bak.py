...
class RAFTDepthNormalSafe4xDPT5(nn.Module):
    def __init__(self, cfg):
        ...
        self.safe_gru = ConvGRU(self.hidden_dims[2], 8 + self.hidden_dims[1] * (self.n_gru_layers > 1))
        # self.safe_up4flow = nn.Sequential(
        #     nn.ReLU(),
    ...
    def _initialize_weights(self):
        ...
        # for m in self.safe_up4flow.modules():
    ...
    def forward(self, vit_features, **kwargs):
        ...
        safe_pred = checkpoint(self.safe_predictor, upflow4(feature_map))
        ...
        safe_net_list = [upflow4(x) for x in net_list]
        safe_inp_list = [[upflow4(xi) for xi in x] for x in inp_list]
        safe_coords0, safe_coords1 = self.initialize_flow_safe(safe_net_list[0])
        ...
        for itr in range(self.iters):
            ...
            if self.single_step_safe:
                if itr == self.iters - 1:
                    safe_gru_flow = torch.cat([flow_up, safe_pred], dim=1)
                    net = self.safe_gru(safe_net_list[0], *(safe_inp_list[0]), safe_gru_flow, interp(safe_net_list[1], safe_net_list[0]))
                ...
                safe_gru_flow = torch.cat([flow_up, safe_flow], dim=1)
                # net = self.safe_gru(net_list[0], *(inp_list[0]), safe_gru_flow, interp(net_list[1], net_list[0]))
                net = checkpoint(self.safe_gru, safe_net_list[0], *(safe_inp_list[0]), safe_gru_flow,
                                 interp(safe_net_list[1], safe_net_list[0]))
                ...
                # final_safe_pred = checkpoint(self.safe_up4flow, final_safe_pred)
                safe_predictions.append(final_safe_pred)
        ...
        # final_safe_pred = self.seg_aggregation(safe_preds_concat)
        # safe_predictions.append(final_safe_pred)
