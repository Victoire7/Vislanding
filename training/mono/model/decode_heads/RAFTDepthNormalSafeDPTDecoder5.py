...
class BasicMultiUpdateBlock(nn.Module):
    def __init__(self, args, hidden_dims=[], out_dims=2):
        ...
        # a = encoder_output_dim + hidden_dims[1] * (self.n_gru_layers > 1)
        # b = hidden_dims[0] * (self.n_gru_layers == 3) + hidden_dims[2]
        # c = hidden_dims[1]
...
class Token2Feature(nn.Module):
    ...
    def __init__(self, vit_channel, feature_channel, scale_factor, use_cls_token=True, num_register_tokens=0):
        ...
        self.scale_factor = scale_factor
        self.readoper = Readout(in_features=vit_channel, use_cls_token=use_cls_token,
...
class EncoderFeature(nn.Module):
    def __init__(self, vit_channel, num_ch_dec=[256, 512, 1024, 1024], use_cls_token=True, num_register_tokens=0):
        ...
        self.read_3 = Token2Feature(self.vit_channel, self.num_ch_dec[3], scale_factor=1, use_cls_token=use_cls_token,
                                    num_register_tokens=num_register_tokens)
        self.read_2 = Token2Feature(self.vit_channel, self.num_ch_dec[2], scale_factor=1, use_cls_token=use_cls_token,
                                    num_register_tokens=num_register_tokens)
        self.read_1 = Token2Feature(self.vit_channel, self.num_ch_dec[1], scale_factor=2, use_cls_token=use_cls_token,
                                    num_register_tokens=num_register_tokens)
        self.read_0 = Token2Feature(self.vit_channel, self.num_ch_dec[0], scale_factor=7 / 2,
                                    use_cls_token=use_cls_token, num_register_tokens=num_register_tokens)
...
class AggregationNetwork(nn.Module):
    def __init__(self, in_channels=8, mid_channels=32, out_channels=2):
...
class ASPPHead(nn.Module):
    def __init__(self, in_ch=48, out_ch=2):
        super().__init__()
        # 【修复Bug】: 移除了硬编码的48，改为动态接收 in_ch
        self.conv0 = nn.Conv2d(in_ch, in_ch, kernel_size=3, padding=1)
        self.relu = nn.ReLU(inplace=True)
        self.conv1 = nn.Conv2d(in_ch, out_ch, 1, dilation=1)
        self.conv2 = nn.Conv2d(in_ch, out_ch, 3, dilation=6, padding=6)
        self.conv3 = nn.Conv2d(in_ch, out_ch, 3, dilation=12, padding=12)
        self.conv4 = nn.Conv2d(in_ch, out_ch, 3, dilation=18, padding=18)
    ...
    def init_weights(self):
        ...
        nn.init.orthogonal_(self.fuse.weight)
        if self.fuse.bias is not None:
            nn.init.constant_(self.fuse.bias, 0)
        nn.init.normal_(self.conv0.weight, mean=0, std=0.01)
    ...
    def forward(self, x):
        ...
        feat3 = self.conv3(x)
        feat4 = self.conv4(x)
        gap_feat = F.interpolate(self.gap(x), size=x.shape[2:], mode='bilinear', align_corners=True)
        return self.fuse(torch.cat([feat1, feat2, feat3, feat4, gap_feat], dim=1))
...
class GeoSemanticFusion(nn.Module):
    """
    模块化开关 1: 2D-3D 几何语义交叉注意力融合 (兼容多门控特征列表)
    """
    def __init__(self, in_2d_channels, in_3d_channels, reduction=4):
        super().__init__()
        mid_channels = max(in_2d_channels // reduction, 16)
        
        # 3D特征提取分支
        self.spatial_gate = nn.Sequential(
            nn.Conv2d(in_3d_channels, mid_channels, kernel_size=3, padding=1),
            nn.BatchNorm2d(mid_channels),
            nn.ReLU(inplace=True),
            nn.Conv2d(mid_channels, in_2d_channels, kernel_size=1),
            nn.Sigmoid()
        )
        
        # 融合后投影
        self.fuse_proj = nn.Conv2d(in_2d_channels * 2, in_2d_channels, kernel_size=3, padding=1)

    def forward(self, feat_2d_list, feat_3d):
        # 计算 3D 空间注意力掩码
        attn_map = self.spatial_gate(feat_3d)
        
        # feat_2d_list 包含了 cz, cr, cq 三个 tensor，遍历它们分别进行融合
        out_list = []
        for feat_2d in feat_2d_list:
            # 强化 2D 特征中的平坦/无障碍区域
            enhanced_2d = feat_2d * attn_map
            # 拼接并融合，维度保持不变
            out = self.fuse_proj(torch.cat([feat_2d, enhanced_2d], dim=1))
            out_list.append(out)
            
        return out_list


class SafeZoneRefiner(nn.Module):
    """
    模块化开关 2: 最终 3D 预测引导的边界细化 (3D-APR)
    在迭代结束后，利用最终的精准深度和法向量来修正安全区的边界。
    """
    def __init__(self, safe_channels=2, depth_channels=1, norm_channels=3):
        super().__init__()
        in_channels = safe_channels + depth_channels + norm_channels
        ...
        self.refine_net = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv2d(32, safe_channels, kernel_size=1)
        )

    def forward(self, safe_pred, depth_final, norm_final):
        # 将最终的预测结果对齐到相同分辨率
        if depth_final.shape[2:] != safe_pred.shape[2:]:
            depth_final = F.interpolate(depth_final, size=safe_pred.shape[2:], mode='bilinear', align_corners=True)
            norm_final = F.interpolate(norm_final, size=safe_pred.shape[2:], mode='bilinear', align_corners=True)
            
        fusion_input = torch.cat([safe_pred, depth_final, norm_final], dim=1)
        residual = self.refine_net(fusion_input)
        return safe_pred + residual
...
class RAFTDepthNormalSafeDPT5(nn.Module):
    def __init__(self, cfg):
        ...
        self.token2feature = EncoderFeature(self.in_channels[0], self.feature_channels, self.use_cls_token,
        ...
        # ---------------- 方案1：多尺度ASPP分割头开关 ----------------
        self.use_multiscale_safe_head = cfg.get('use_multiscale_safe_head', False)
        if self.use_multiscale_safe_head:
            # 融合来自ViT全尺度的特征 (1/14, 1/7, 1/4)
            ms_dim = sum(self.feature_channels)
            self.ms_fusion = nn.Sequential(
                nn.Conv2d(ms_dim, 256, kernel_size=1),
                nn.BatchNorm2d(256),
                nn.ReLU(inplace=True),
            ...
            # 使用强大的 ASPP 替代简单的 Conv
            self.safe_predictor = ASPPHead(in_ch=128, out_ch=2)
        else:
            self.safe_predictor = nn.Sequential(
                nn.Conv2d(self.used_res_channel, 128, kernel_size=3, padding=1),
                nn.ReLU(inplace=True),
        ...
        self.use_ASPP = cfg.get('use_ASPP', False)
        if not self.use_ASPP:
            ...
            self.safe_head = ASPPHead(in_ch=48, out_ch=2)

        self.flow_ablation = np.array(cfg.get('flow_ablation', [True, True, True]))
        flow_channels = np.array([2, 4, 48])
        
        # ---------------- 方案3：特征调制 (SPADE/SFT) 开关 ----------------
        self.use_feature_modulation = cfg.get('use_feature_modulation', False)
        if self.use_feature_modulation:
            self.safe_gru_input_channel = 2 # 调制后不需要再Concat深度法向流
            # 使用6通道(深度+法向+置信度)来生成缩放和偏移参数
            self.sft_gamma = nn.Conv2d(6, 2, kernel_size=3, padding=1)
            self.sft_beta = nn.Conv2d(6, 2, kernel_size=3, padding=1)
        else:
            self.safe_gru_input_channel = 2 + flow_channels[self.flow_ablation].sum()
            
        self.safe_flow_channel = 2 + flow_channels[:-1][self.flow_ablation[:-1]].sum()
        self.safe_gru = ConvGRU(self.hidden_dims[2], self.safe_gru_input_channel)
        # ------------------------------------------------------------------

        self.with_refine = cfg.get('with_refine', False)
        if self.with_refine:
            self.safe_refine_net = nn.Conv2d(2, 2, kernel_size=1)
        self.with_safe_neck = cfg.get('with_safe_neck', False)
        if self.with_safe_neck:
            self.safe_aggregation = AggregationNetwork(in_channels=self.iters * 2)
        ...
        self.relu = nn.ReLU(inplace=True)
        self.single_step_safe = cfg.get('single_step_safe', False)

                        # ----------------- 论文创新点模块化开关 -----------------
        self.use_geo_fusion = cfg.get('use_geo_fusion', False)
        self.use_3d_refiner = cfg.get('use_3d_refiner', False)
        self.safe_gru_input_channel = 2 + flow_channels[self.flow_ablation].sum()

        if self.use_geo_fusion:
            # inp_list[0] 的通道数由 context_zqr_convs 决定，通常是 hidden_dims 的三分之一
            # 这里我们对 2D context feature 和 3D flow 进行深度融合
            context_dim = self.hidden_dims[0]
            self.safe_geo_fusion_module = GeoSemanticFusion(
                in_2d_channels=context_dim, 
                in_3d_channels=self.safe_gru_input_channel
            )

        if self.use_3d_refiner:
            # 接收 final safe pred (2), final depth (1), final normal (3)
            self.safe_3d_refiner = SafeZoneRefiner(safe_channels=2, depth_channels=1, norm_channels=3)
        self._initialize_weights()

    def _initialize_weights(self):
        # ... [保留原本权重初始化逻辑] ...
        for m in self.safe_predictor.modules():
            if isinstance(m, nn.Conv2d):
        ...
        # 调制模块初始化
        if self.use_feature_modulation:
            init.constant_(self.sft_gamma.weight, 0)
            init.constant_(self.sft_gamma.bias, 0)
            init.constant_(self.sft_beta.weight, 0)
            init.constant_(self.sft_beta.bias, 0)
    ...
    def run_update_block2(self, net_list, inp_list, corr, flow, iter32, iter16):
        return self.update_block(net_list, inp_list, corr, flow, iter32=iter32, iter16=iter16)
    ...
    def forward(self, vit_features, **kwargs):
        ...
        # ---------------- 方案1：多尺度预测前向传播 ----------------
        if self.use_multiscale_safe_head:
            # 将不同尺度的特征强制上采样到同一尺度 (1/4)
            target_size = encoder_features[3].shape[2:]
            x_14_1 = F.interpolate(encoder_features[0], size=target_size, mode='bilinear', align_corners=True)
            x_14_2 = F.interpolate(encoder_features[1], size=target_size, mode='bilinear', align_corners=True)
            x_7    = F.interpolate(encoder_features[2], size=target_size, mode='bilinear', align_corners=True)
            x_4    = encoder_features[3]
            
            fused_ms = torch.cat([x_14_1, x_14_2, x_7, x_4], dim=1)
            fused_feat = self.ms_fusion(fused_ms)
            safe_pred = self.safe_predictor(fused_feat)
        else:
            safe_pred = self.safe_predictor(feature_map)
        # -----------------------------------------------------------
        ...
        safe_coords1 = safe_coords1 + safe_pred
        ...
        for itr in range(self.iters):
            ...
            safe_flow = safe_coords1 - safe_coords0
            if self.single_step_safe:
                if itr == self.iters - 1:
                    safe_gru_flow = torch.cat([coords1 - coords0, safe_pred], dim=1)
                    net = self.safe_gru(net_list[0], *(inp_list[0]), safe_gru_flow, interp(net_list[1], net_list[0]))
                    final_safe_pred = self.safe_head(net)  
                    final_safe_pred = upflow4(final_safe_pred)
                    safe_predictions.append(final_safe_pred)
            else:
                dn_flow = coords1 - coords0
                safe_gru_flow = None
                if self.safe_flow_channel == 2:
                    safe_gru_flow = safe_flow
                elif self.safe_flow_channel == 4:
                    safe_gru_flow = torch.cat([dn_flow[:, :2], safe_flow], dim=1)
                elif self.safe_flow_channel == 6:
                    safe_gru_flow = torch.cat([dn_flow[:, 2:], safe_flow], dim=1)
                elif self.safe_flow_channel == 8:
                    safe_gru_flow = torch.cat([dn_flow, safe_flow], dim=1)

                # ================= 创新点 1: Geo-Semantic Fusion =================
                context_feat = inp_list[0]
                if self.use_geo_fusion:
                    # 此时 context_feat 是一个 list，模块内部会处理并返回融合后的 list
                    context_feat = self.safe_geo_fusion_module(context_feat, safe_gru_flow)
                # ===============================================================

                if self.flow_ablation[2]:
                    # 修复：移除多余的逗号，正确解包 list 为 cz, cr, cq 三个独立参数
                    net = self.safe_gru(net_list[0], *context_feat, safe_gru_flow, interp(net_list[1], net_list[0]))
                else:
                    # 同样修复这里的解包
                    net = self.safe_gru(net_list[0], *context_feat, safe_gru_flow)
                
                safe_delta_flow = checkpoint(self.safe_head, net)
                safe_coords1 = safe_coords1 + safe_delta_flow
                final_safe_pred = safe_coords1 - safe_coords0
                final_safe_pred = self.upsample_flow(final_safe_pred, up_mask)
                
                if self.with_refine:
                    final_safe_pred = self.safe_refine_net(final_safe_pred)
                
                safe_predictions.append(final_safe_pred)

        # 循环结束后的聚合逻辑
        if self.with_safe_neck:
            safe_preds_stack = torch.stack(safe_predictions, dim=1)  
            safe_preds_concat = safe_preds_stack.view(B, -1, 616, 1064) 
            final_safe_pred = self.safe_aggregation(safe_preds_concat)
            safe_predictions.append(final_safe_pred)

        # ================= 创新点 2: 3D-Guided Post-Refinement =================
        if self.use_3d_refiner and not self.single_step_safe:
            # 提取最后一次迭代的深度和法向量预测
            final_depth = flow_predictions[-1][:, :1, :, :] # (B, 1, H, W)
            final_normal = normal_outs[-1]                  # (B, 4, H, W)，取前3个通道
            final_normal_xyz = final_normal[:, :3, :, :]
            
            # 使用最终的 3D 预测结果对安全区进行边界和物理约束细化
            refined_safe_pred = self.safe_3d_refiner(final_safe_pred, final_depth, final_normal_xyz)
            safe_predictions.append(refined_safe_pred)
            final_safe_pred = refined_safe_pred

        outputs = dict(
            prediction=flow_predictions[-1],
            predictions_list=flow_predictions,
            safe_prediction=final_safe_pred,
