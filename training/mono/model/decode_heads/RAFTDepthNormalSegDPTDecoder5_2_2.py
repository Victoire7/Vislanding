...
def coords_grid(batch, ht, wd):
    ...
    coords = (
    torch.zeros((ht, wd)), torch.zeros((ht, wd)), torch.zeros((ht, wd)), torch.zeros((ht, wd)), torch.zeros((ht, wd)),
    torch.zeros((ht, wd)), torch.zeros((ht, wd)), torch.zeros((ht, wd)))
...
class FlowHead(nn.Module):
    def __init__(self, input_dim=128, hidden_dim=256, output_dim_depth=2, output_dim_norm=4, output_dim_safe=2):
        ...
        self.conv1s = nn.Conv2d(input_dim + 6, hidden_dim // 2, 3, padding=1)
        self.conv2s = nn.Conv2d(hidden_dim // 2, output_dim_safe, 3, padding=1)

    def forward(self, x):
        depth = self.conv2d(self.relu(self.conv1d(x)))
        normal = self.conv2n(self.relu(self.conv1n(x)))
        safe_input = torch.cat((x, depth, normal), dim=1)
        safe_pre = self.conv2s(self.relu(self.conv1s(safe_input)))
        return torch.cat((depth, normal, safe_pre), dim=1)
...
class BasicMultiUpdateBlock(nn.Module):
    def __init__(self, args, hidden_dims=[], out_dims=2):
        ...
        encoder_output_dim = 8  # no corr volume
...
class SegmentationHead(nn.Module):
    def __init__(self, in_channels=6, mid_channels=32, out_channels=2):
...
class AggregationNetwork(nn.Module):
    def __init__(self, in_channels=8, mid_channels=16, out_channels=2):
...
class SafePredictor(nn.Module):
    def __init__(self, in_channels=6, mid_channels=128, out_channels=2):
        super(SafePredictor, self).__init__()
        self.conv1 = nn.Conv2d(in_channels, mid_channels, kernel_size=3, padding=1)
        self.relu = nn.ReLU(inplace=True)
...
class RAFTDepthNormalSegDPT5_2_2(nn.Module):
    def __init__(self, cfg):
        ...
        # self.seg_predictor = SegmentationHead()
        # self.seg_aggregation = AggregationNetwork()
        self.safe_predictor = SafePredictor(in_channels=self.used_res_channel)
        ...
        self.update_block = BasicMultiUpdateBlock(cfg, hidden_dims=self.hidden_dims, out_dims=8)
    ...
    def forward(self, vit_features, **kwargs):
        ...
        depth_init = torch.cat((depth_pred, depth_confidence_map, normal_pred, safe_pred), dim=1)
        # *********************************************************************************
        ...
        if self.training:
            low_resolution_init = [self.clamp(depth_init[:, :1] * self.regress_scale + self.max_val),
                                   depth_init[:, 1:2], norm_normalize(depth_init[:, 2:-2].clone()), depth_init[:, -2:]]
            ...
            normal_outs = [norm_normalize(init_depth[:, 2:-2].clone())]
            safe_predictions = [init_depth[:, -2:]]
        ...
        for itr in range(self.iters):
            ...
            normal_outs.append(norm_normalize(flow_up[:, 2:-2].clone()))
            safe_predictions.append(flow_up[:, -2:])
