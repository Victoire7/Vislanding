...
def coords_grid(batch, ht, wd):
    ...
    torch.zeros((ht, wd)), torch.zeros((ht, wd)))
...
class FlowHead(nn.Module):
    def __init__(self, input_dim=128, hidden_dim=256, output_dim_depth=2, output_dim_norm=4, output_dim_safe=1):
...
class BasicMultiUpdateBlock(nn.Module):
    def __init__(self, args, hidden_dims=[], out_dims=2):
        ...
        encoder_output_dim = 7  # no corr volume
...
class SafePredictor(nn.Module):
    def __init__(self, in_channels=6, mid_channels=128, out_channels=1):
...
class RAFTDepthNormalSegDPT5_2(nn.Module):
    ...
    def forward(self, vit_features, **kwargs):
        ...
        if self.training:
            low_resolution_init = [self.clamp(depth_init[:, :1] * self.regress_scale + self.max_val),
                                   depth_init[:, 1:2], norm_normalize(depth_init[:, 2:-1].clone()), depth_init[:, -1:]]
            ...
            normal_outs = [norm_normalize(init_depth[:, 2:-1].clone())]
            safe_predictions = [init_depth[:, -1:]]
        ...
        for itr in range(self.iters):
            ...
            normal_outs.append(norm_normalize(flow_up[:, 2:-1].clone()))
            safe_predictions.append(flow_up[:, -1:])
