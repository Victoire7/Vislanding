...
class ASPPHead(nn.Module):
    def __init__(self, in_ch=48, out_ch=2):
        ...
        self.gap = nn.Sequential(
            ...
            nn.Conv2d(in_ch, out_ch, 1),  # 关键修改点
            nn.ReLU(inplace=True)
        )
    ...
    def forward(self, x):
        ...
        feat3 = self.conv3(x)
        feat4 = self.conv4(x)
        gap_feat = F.interpolate(self.gap(x), size=x.shape[2:], mode='bilinear')
...
class RAFTDepthNormalSafeDPT5(nn.Module):
    def __init__(self, cfg):
        ...
        # self.safe_gru16 = ConvGRU(self.hidden_dims[1], self.hidden_dims[0] + self.hidden_dims[2])
        ...
        self.context_feature_encoder = ContextFeatureEncoder(self.feature_channels,
        ...
        self.relu = nn.ReLU(inplace=True)
        self.single_step_safe = cfg.single_step_safe
        self._initialize_weights()

    def _initialize_weights(self):
        # 初始化 safe_predictor 的参数
        for m in self.safe_predictor:
            if isinstance(m, nn.Conv2d):
        ...
        # 初始化 safe_head 的参数
        if isinstance(self.safe_head, nn.Sequential):
            for m in self.safe_head:
                if isinstance(m, nn.Conv2d):
                    init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
                    if m.bias is not None:
                        init.constant_(m.bias, 0)
        # 初始化 safe_gru 的参数（假设 ConvGRU 是一个自定义模块）
        for m in self.safe_gru.modules():
            if isinstance(m, nn.Conv2d):
                init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
                if m.bias is not None:
                    init.constant_(m.bias, 0)
        # for m in self.safe_gru16.modules():
        #     if isinstance(m, nn.Conv2d):
        #         init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
        #         if m.bias is not None:
        #             init.constant_(m.bias, 0)
        if self.with_refine:
            init.kaiming_normal_(self.safe_refine_net.weight, mode='fan_out', nonlinearity='relu')
            if self.safe_refine_net.bias is not None:
                init.constant_(self.safe_refine_net.bias, 0)
    ...
    def forward(self, vit_features, **kwargs):
        ...
        # self.max_val = int(self.max_val * torch.max(kwargs['scale']).cpu().numpy()) + 1
        # self.regress_scale = int(self.max_val / 2) + 1
        ...
        if self.use_cls_token == True:
            vit_features = [[ft[:, 1 + num_register_tokens:, :].view(B, H, W, self.in_channels[0]), \
                             ft[:, 0:1 + num_register_tokens, :].view(B, 1, 1,
                                                                      self.in_channels[0] * (1 + num_register_tokens))]
        ...
        encoder_features = checkpoint(self.token2feature, vit_features)  # 1/14, 1/14, 1/7, 1/4
        ...
        ref_feat = checkpoint(self.decoder_mono, encoder_features)  # now, 1/4 for depth
        ...
        depth_pred, binmap = checkpoint(self.regress_depth, feature_map)  # regress bin for depth
        # normal_pred = self.pred_normal(feature_map, normal_confidence_map) # mlp for normal
        normal_pred = checkpoint(self.pred_normal, feature_map, normal_confidence_map)  # mlp for normal
        # safe_pred = checkpoint(self.safe_predictor, feature_map)
        ...
        if self.training:
            ...
            low_resolution_init = []
            safe_predictions = []
        safe_net0 = copy.deepcopy(net_list[0])
        for itr in range(self.iters):
            ...
            if self.n_gru_layers >= 2 and self.slow_fast_gru:  # Update low-res GRU and mid-res GRU
                ...
                net_list = checkpoint(self.run_update_block, net_list, inp_list, self.n_gru_layers == 3, True, False,
                                      False)
            ...
            # net_list, up_mask, delta_flow = checkpoint(self.run_update_block2, net_list, inp_list, None, flow, self.n_gru_layers==3, self.n_gru_layers>=2)
            ...
            if self.single_step_safe:
                if itr == self.iters - 1:
                    ...
                    final_safe_pred = self.safe_head(net)  # new flow
                ...
                # safe_gru_flow = torch.cat([coords1 - coords0, safe_flow], dim=1)
                # net = self.safe_gru(net_list[0], *(inp_list[0]), safe_gru_flow, interp(net_list[1], net_list[0]))
                if self.flow_ablation[2]:
                    # net = checkpoint(self.safe_gru, net_list[0], *(inp_list[0]), safe_gru_flow, interp(net_list[1], net_list[0]))
                    # net_list_1 = self.safe_gru16(net_list[1], *(inp_list[1]), interp(pool2x(net_list[0]), net_list[1]),
                    #                              interp(net_list[2], net_list[1]))
                    # # print(net_list_1)
                    net = self.safe_gru(net_list[0], *(inp_list[0]), safe_gru_flow, interp(net_list[1], net_list[0]))
                    # safe_net0 = self.safe_gru(safe_net0, *(inp_list[0]), safe_gru_flow, interp(net_list[1], safe_net0))
                else:
                    # net = checkpoint(self.safe_gru, net_list[0], *(inp_list[0]), safe_gru_flow)
                    net = self.safe_gru(net_list[0], *(inp_list[0]), safe_gru_flow)
                safe_delta_flow = checkpoint(self.safe_head, net)
                # safe_delta_flow = self.safe_head(safe_net0)
                safe_coords1 = safe_coords1 + safe_delta_flow
                final_safe_pred = safe_coords1 - safe_coords0
                # final_safe_pred2 = upflow4(final_safe_pred)
                final_safe_pred = self.upsample_flow(final_safe_pred, up_mask)
                if self.with_refine:
                    final_safe_pred = self.safe_refine_net(final_safe_pred)
                safe_predictions.append(final_safe_pred)
            # safe_predictions.append(self.seg_predictor(flow_up))
        if self.with_safe_neck:
            safe_preds_stack = torch.stack(safe_predictions, dim=1)  # 形状: (B, 4, 2, H, W)
            safe_preds_concat = safe_preds_stack.view(B, -1, 616, 1064)  # 形状: (B, 8, H, W)
            final_safe_pred = self.safe_aggregation(safe_preds_concat)
            safe_predictions.append(final_safe_pred)
        # final_safe_pred = None
