...
def weight_reduce_loss(loss,
    ...
    if weight is not None:
        ...
        loss = loss * weight
    if punish_weight is not None:
        loss = loss * punish_weight
...
def cross_entropy(pred,
    ...
    if (avg_factor is None) and reduction == 'mean':
        if class_weight is None:
            ...
            class_weight = torch.cat([class_weight, torch.tensor([0.0]).to(
    ...
    loss = weight_reduce_loss(
        loss.float(), weight=weight, reduction=reduction, avg_factor=avg_factor, punish_weight=punish_weight)
