...
def dice_loss(pred: torch.Tensor,
    ...
    class_weight = torch.ones_like(target)
    class_weight[target == ignore_index] = 0
    a = torch.sum(input * target * class_weight, 1)
    if naive_dice:
        b = torch.sum(input * class_weight, 1)
        c = torch.sum(target * class_weight, 1)
        d = (2 * a + eps) / (b + c + eps)
    else:
        b = torch.sum(input * input * class_weight, 1) + eps
        c = torch.sum(target * target * class_weight, 1) + eps
        d = (2 * a) / (b + c)
