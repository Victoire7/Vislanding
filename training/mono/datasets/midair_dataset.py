...
class MidAirDataset(BaseDataset):
    def __init__(self, cfg, phase, **kwargs):
        super(MidAirDataset, self).__init__(
            cfg=cfg,
            phase=phase,
            **kwargs)
        self.metric_scale = cfg.metric_scale
        self.data_name = 'Midair'
    ...
    def get_data_for_trainval(self, idx: int):
        ...
        filename = os.path.basename(meta_data['rgb'])[:-4] + '.JPEG'
    ...
    def load_sem_label(self, sem_path, depth=None, sky_id=0) -> np.array:
        H, W = depth.shape
    ...
    def load_rgb_depth(self, rgb_path: str, depth_path: str):
        ...
        depth_pic = Image.open(depth_path)
        depth = np.asarray(depth_pic, np.uint16)
        depth.dtype = np.float16
        depth = np.clip(depth, 1, 1250, depth)
        if depth is None:
    ...
    def load_norm_label(self, norm_path, H, W):
        ...
        norm_gt = cv2.resize(norm_gt, (W, H))
