...
class SafeWildUAVDataset(BaseDataset):
    def __init__(self, cfg, phase, **kwargs):
        super(SafeWildUAVDataset, self).__init__(
            cfg=cfg,
            phase=phase,
            **kwargs)
    ...
    def verify_data_alignment(self, data_dict, save_path="debug_alignment.png"):
        """
        可视化验证 input(RGB) 和 safe_target(Mask) 是否对齐
        """
        # 1. 获取张量并放到 CPU
        img_tensor = data_dict['input'].detach().cpu()
        safe_mask = data_dict['safe_target'].detach().cpu().squeeze()
        
        # 2. 逆归一化 (反向操作: img * std + mean)
        # 注意：这里的 mean 和 std 必须与你 Pipeline 中的完全一致
        mean = torch.tensor([123.675, 116.28, 103.53]).view(3, 1, 1)
        std = torch.tensor([58.395, 57.12, 57.375]).view(3, 1, 1)
        
        img_unnorm = img_tensor * std + mean
        # 限制在 0-255 范围内并转为 numpy 数组 (H, W, C)
        img_np = img_unnorm.clamp(0, 255).byte().permute(1, 2, 0).numpy()
        mask_np = safe_mask.numpy()
        ...
        axes[0].set_title("Augmented RGB Input")
        axes[0].axis('off')
        
        # 图2：安全区掩码
        axes[1].imshow(mask_np, cmap='gray')
        axes[1].set_title("Safe Target Mask")
        axes[1].axis('off')
        
        # 图3：叠加显示 (红色半透明表示安全区，或者用其他颜色)
        axes[2].imshow(img_np)
        axes[2].imshow(mask_np, cmap='jet', alpha=0.4) # alpha控制透明度
        ...
        plt.close()
        print(f"验证图像已保存至 {save_path}，请打开人工检查。")
    ...
    def get_data_for_trainval(self, idx: int):
        ...
        curr_rgb, curr_depth, curr_normal, curr_sem, curr_cam_model, curr_safe = data_batch['curr_rgb'], data_batch['curr_depth'], \
            data_batch['curr_normal'], data_batch['curr_sem'], data_batch['curr_cam_model'], data_batch['curr_safe']
        ...
        rgbs, depths, intrinsics, cam_models, normals, other_labels, transform_paras = self.img_transforms(
            ...
            normals=[curr_normal, ],
            other_labels=[curr_sem, curr_stereo_depth, curr_safe],
        ...
        safe_target[(safe_target != 0) & (safe_target != 1)] = 2
        data = dict(input=rgbs[0],
                    weak_input=torch.zeros(rgbs[0].shape),
                    target=depth_out,
        ...
        # self.verify_data_alignment(data, save_path=f"/home/vector/Tan/xunfeidan/Metric3D/ann_vis/debug_alignment_{filename}.png")
        return data
    ...
    def get_data_for_test(self, idx: int):
        ...
        # data_batch = self.load_batch_hdf5(sample_id)
        # load data
        curr_rgb, curr_depth, curr_normal, curr_cam_model, curr_safe = data_batch['curr_rgb'], data_batch['curr_depth'], \
            data_batch['curr_normal'], data_batch['curr_cam_model'], data_batch['curr_safe']
    ...
    def load_safe_label(self, safe_path):
        ...
        # image_array = np.array(image)  # 形状为 (H, W, 3)
    ...
    def load_data_path(self, meta_data):
        ...
        curr_safe_path = os.path.join(self.data_root, meta_data['safe_ann'])
        ...
        curr_depth_mask_path = os.path.join(self.depth_mask_root, meta_data['depth_mask']) \
            if self.depth_mask_root is not None and ('depth_mask' in meta_data) and (
                    meta_data['depth_mask'] is not None) \
            else None
