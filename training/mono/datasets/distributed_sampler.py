...
def build_dataset_n_sampler_with_cfg_semi(cfg, phase):
    # build data array, similar datasets are organized in the same group
    datasets_array = build_data_array(cfg, phase)
    # concatenate datasets with torch.utils.data.ConcatDataset methods
    dataset_merge = concatenate_datasets(datasets_array)
    # customerize sampler
    custom_sampler = CustomerMultiDataSamplerSameRatio(cfg, dataset_merge, phase)
    return dataset_merge, custom_sampler


class CustomerMultiDataSamplerSameRatio(torch.utils.data.Sampler):
    """
    Custom sampler to ensure each dataset is sampled in a 1:1 ratio per batch.
    Each dataset is treated as an individual group and expanded to the same size.
    """
    ...
    def __len__(self):
        return self.each_gpu_size

    def create_samplers(self):
        # Collect all individual datasets with their global indices
        all_datasets_info = []
        current_start = 0
        for group in self.dataset_merge.datasets:
            if isinstance(group, torch.utils.data.ConcatDataset):
                group_cumulative = [0] + group.cumulative_sizes
                for i, sub_dataset in enumerate(group.datasets):
                    start = current_start + group_cumulative[i]
                    end = current_start + group_cumulative[i+1]
                    all_datasets_info.append((sub_dataset, start, end))
                current_start += group_cumulative[-1]
            else:
                all_datasets_info.append((group, current_start, current_start + len(group)))
                current_start += len(group)

        # Calculate maximum dataset size
        max_dataset_size = max(end - start for _, start, end in all_datasets_info)
        num_datasets = len(all_datasets_info)

        self.indices_expand_mat = []
        for dataset, start, end in all_datasets_info:
            original_size = end - start
            cp_times = max_dataset_size / original_size
            integer_part = int(cp_times)
            remainder = max_dataset_size - integer_part * original_size

            original_indices = list(range(start, end))
            expanded_indices = original_indices * integer_part
            if remainder > 0:
                random.seed(self.random_seed_cp)
                expanded_indices += random.sample(original_indices, remainder)
            ...
            self.indices_expand_mat.append(expanded_indices)
            if main_process():
                self.logger.info(f'Dataset "{dataset.data_name}" expanded from {original_size} to {len(expanded_indices)} samples.')

        # Interleave datasets to ensure 1:1 sampling per batch
        if self.phase == 'train':
            self.sample_indices_array = np.array(self.indices_expand_mat).T.reshape(-1)
        else:
            # For non-training phases, use original order without expansion
            self.sample_indices_array = np.concatenate([list(range(start, end)) for _, start, end in all_datasets_info])

        # Adjust for distributed training
        self.total_indices_size = len(self.sample_indices_array)
        self.each_gpu_size = int(np.ceil(self.total_indices_size / self.world_size))
        self.total_dist_size = self.each_gpu_size * self.world_size

        # Handle cases where total indices are not divisible by world_size
        diff_size = self.total_dist_size - self.total_indices_size
        if diff_size > 0:
            self.sample_indices_array = np.append(self.sample_indices_array, self.sample_indices_array[:diff_size])

        # Update random seeds for next epoch
...
def build_data_array(cfg, phase):
    ...
    for group_i in dataname_array:
        ...
        for data_i in group_i:
            ...
            if "Unlabel" in data_i_cfg_name and phase in unlabel_ignore_phase:
                continue
