...
class NewUnlabelSafeWildUAVDataset(BaseDataset):
    def __init__(self, cfg, phase, **kwargs):
        super(NewUnlabelSafeWildUAVDataset, self).__init__(
            cfg=cfg,
            phase=phase,
            **kwargs)
        self.metric_scale = cfg.metric_scale
        self.transforms_cfg = cfg.data[phase]['strong_pipeline']
        self.transforms_cfg_weak = cfg.data[phase]['weak_pipeline']
        self.img_transforms_semi = img_transform.SemiCompose(self.build_data_transforms(),
                                                             self.build_data_transforms_weak())

    def build_data_transforms_weak(self):
        transforms_list = []
        for transform in self.transforms_cfg_weak:
    ...
    def get_data_for_trainval(self, idx: int):
        ...
        curr_rgb, curr_depth, curr_normal, curr_sem, curr_cam_model, curr_safe = data_batch['curr_rgb'], data_batch[
            'curr_depth'], \
        ...
        rgbs, depths, intrinsics, cam_models, normals, other_labels, transform_paras, weak_rgbs, weak_depths, weak_intrinsics, weak_cam_models, weak_normals, weak_other_labels, weak_transform_paras = self.img_transforms_semi(
            images=[curr_rgb, ],
            labels=[curr_depth, ],
    ...
    def load_batch(self, meta_data, data_path):
        ...
        curr_stereo_depth = self.load_stereo_depth_label(data_path['disp_path'], H=curr_rgb.shape[0],
                                                         W=curr_rgb.shape[1])
        curr_safe = self.load_safe_label(data_path['safe_path'], H=curr_rgb.shape[0],
                                         W=curr_rgb.shape[1])
    ...
    def load_rgb_depth(self, rgb_path: str, depth_path: str):
        ...
        if depth_path is None:
            depth = np.ones((rgb.shape[0], rgb.shape[1])).astype(np.float32) * -1
            return rgb, depth
    ...
    def load_safe_label(self, safe_path, H, W):
