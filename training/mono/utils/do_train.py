...
from mono.utils.running import build_lr_schedule_with_cfg, build_optimizer_with_cfg, load_ckpt, save_ckpt, save_best_ckpt
...
def do_train(local_rank: int, cfg: dict):
    ...
    for name, param in model.named_parameters():
        if "lora" in name or 'decode' in name:
            print(name)
        ...
        while step < max_iters:
            ...
            if main_process():
                if ((step+1) % save_interval == 0) or ((step+1)==max_iters):
                    ...
                    save_best_ckpt(cfg, model, optimizer, lr_scheduler, step + 1, epoch, val_err)
            ...
            # validate the model
                ...
                if main_process():
                    if (((step+acc_batch)//acc_batch) % save_interval == 0) or (((step+acc_batch)//acc_batch)==max_iters):
                        ...
                        save_best_ckpt(cfg, model, optimizer, lr_scheduler, ((step+acc_batch)//acc_batch), epoch, val_err, loss_scaler=loss_scaler)
...
def validate(cfg, iter, model, val_dataloader, tb_logger):
    ...
    # val_dataloader.dataset.data_name = 'mid-air'
