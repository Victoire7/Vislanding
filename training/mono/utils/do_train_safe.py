...
def do_train_safe(local_rank: int, cfg: dict):
            ...
            if (step + 1 - start_iter) % acc_batch == 0:
                ...
                if main_process():
                    if (((step + acc_batch) // acc_batch) % save_interval == 0) or (
                        ...
                        save_best_ckpt(cfg, model, optimizer, lr_scheduler, ((step + acc_batch) // acc_batch), epoch,
                                       val_err, seg_val_err,loss_scaler=loss_scaler)
...
def validate_multiple_dataset(cfg, iter, model, val_dataloaders, tb_logger):
    ...
    for val_dataloader in val_dataloaders:
        val_err, seg_metrics = validate(cfg, iter, model, val_dataloader, tb_logger)
