...
class SemiCompose(object):
    def __init__(self, strong_transforms, weak_transforms):
        self.strong_transforms = strong_transforms
        self.weak_transforms = weak_transforms

        # 定位weak_pipeline中的ToTensor步骤索引
        self.weak_to_tensor_index = None
        for i, trans in enumerate(self.weak_transforms):
            if trans.__class__.__name__ == 'ToTensor':
                self.weak_to_tensor_index = i
                break
        if self.weak_to_tensor_index is None:
            raise ValueError("ToTensor not found in weak_transforms")

        self.weak_pre_steps_count = self.weak_to_tensor_index

    def __call__(self, images, labels, intrinsics, cam_models=None, normals=None, other_labels=None,
                 transform_paras=None):
        # 处理strong_pipeline并保存中间数据
        strong_data = (images, labels, intrinsics, cam_models, normals, other_labels, transform_paras)
        weak_intermediate_data = None

        for i, trans in enumerate(self.strong_transforms):
            strong_data = trans(*strong_data)
            # 检查是否到达weak_pipeline的ToTensor之前的最后一个步骤
            if i == self.weak_pre_steps_count - 1:
                weak_intermediate_data = [copy.deepcopy(d) for d in strong_data]

        # 处理weak_pipeline的剩余步骤
        if weak_intermediate_data is None:
            raise RuntimeError(
                "Weak intermediate data not captured. Check if strong_transforms align with weak_pre_steps.")

        weak_data = weak_intermediate_data
        for trans in self.weak_transforms[self.weak_to_tensor_index:]:
            weak_data = trans(*weak_data)
...
class RandomEdgeMask(object):
    ...
    def __call__(self, images, labels, intrinsics, cam_models=None, normals=None, other_labels=None, transform_paras=None):
        assert len(images[0].shape) == 3 and len(labels[0].shape) == 2
        assert labels[0].dtype == np.float32 or labels[0].dtype == np.float64
