...
def load_ckpt(load_path, model, optimizer=None, scheduler=None, strict_match=True, loss_scaler=None):
    ...
    if os.path.isfile(load_path):
        ...
        try:
            ...
            for key in ['depth_model.decoder.update_block.gru08.convz.weight',
                        'depth_model.decoder.update_block.gru08.convr.weight',
                        'depth_model.decoder.update_block.gru08.convq.weight']:
                if key in ckpt_state_dict:
                    old_weight = ckpt_state_dict[key]
                    new_weight = torch.zeros(48, 104, 3, 3)  # 新的权重形状
                    new_weight[:, :102, :, :] = old_weight  # 将旧权重复制到新权重中
                    ckpt_state_dict[key] = new_weight
            model.module.load_state_dict(ckpt_state_dict, strict=strict_match)
...
def save_best_ckpt_semi(cfg, model, optimizer, scheduler, curr_iter=0, curr_epoch=None, val_error=None, seg_val_err=None,
                   loss_scaler=None, prefix=''):
    """
    Save the model, optimizer, lr scheduler.
    """
    logger = logging.getLogger()
    ...
    if curr_iter == cfg.semi_start_iter:
        save_div = os.path.join(cfg.work_dir, f'step{curr_iter}.pth')
        torch.save(ckpt, save_div)

    ckpt_dir = os.path.join(cfg.work_dir, 'ckpt')
    os.makedirs(ckpt_dir, exist_ok=True)

    save_name = os.path.join(ckpt_dir, f'{prefix}_step%08d.pth' % curr_iter)
    saved_ckpts = glob.glob(ckpt_dir + f'/{prefix}_step*.pth')
    torch.save(ckpt, save_name)

    # keep the last 8 ckpts
    if len(saved_ckpts) > 10:
        saved_ckpts.sort()
        os.remove(saved_ckpts.pop(0))

    logger.info(f'Save model: {save_name}')

    best_kpt_dir = os.path.join(cfg.work_dir, f'best_depth_ckpt')
    os.makedirs(best_kpt_dir, exist_ok=True)
    print(val_error)
    key_name = None
    for key in val_error.keys():
        if 'abs_rel' in key:
            key_name = key
            break
    # abs_rel = val_error['WildUAV_eval/abs_rel']
    abs_rel = val_error[key_name]
    best_save_name = os.path.join(best_kpt_dir, f'{prefix}_step%08d_{abs_rel:.4f}.pth' % curr_iter)
    best_saved_ckpts = glob.glob(best_kpt_dir + f'/{prefix}_step*.pth')
    if len(best_saved_ckpts) == 0:
        torch.save(ckpt, best_save_name)
        logger.info(f'Save best {prefix} depth model: {best_save_name}')
    else:
        pre_error = float(best_saved_ckpts[0].split('_')[-1].split('.pth')[0])
        if round(abs_rel, 4) < pre_error:
            os.remove(best_saved_ckpts.pop(0))
            torch.save(ckpt, best_save_name)
            logger.info(f'Save best {prefix} depth model: {best_save_name}, Pre best removed')

    if cfg.with_safe:
        best_safe_kpt_dir = os.path.join(cfg.work_dir, 'best_safe_ckpt')
        os.makedirs(best_safe_kpt_dir, exist_ok=True)
        key_name = None
        for key in seg_val_err.keys():
            if 'mIoU' in key:
                key_name = key
                break
        # abs_rel = val_error['WildUAV_eval/abs_rel']
        miou = seg_val_err[key_name]
        best_safe_save_name = os.path.join(best_safe_kpt_dir, f'{prefix}_step%08d_{miou:.2f}.pth' % curr_iter)
        best_safe_saved_ckpts = glob.glob(best_safe_kpt_dir + f'/{prefix}_step*.pth')
        if len(best_safe_saved_ckpts) == 0:
            torch.save(ckpt, best_safe_save_name)
            logger.info(f'Save best {prefix} safe model: {best_safe_save_name}')
        else:
            pre_miou = float(best_safe_saved_ckpts[0].split('_')[-1].split('.pth')[0])
            if round(miou, 2) > pre_miou:
                os.remove(best_safe_saved_ckpts.pop(0))
                torch.save(ckpt, best_safe_save_name)
                logger.info(f'Save best {prefix} safe model: {best_safe_save_name}, Pre best removed')

def save_best_ckpt(cfg, model, optimizer, scheduler, curr_iter=0, curr_epoch=None, val_error=None, seg_val_err=None,
                   loss_scaler=None):
    """
    Save the model, optimizer, lr scheduler.
    """
    ...
    best_kpt_dir = os.path.join(cfg.work_dir, 'best_depth_ckpt')
    os.makedirs(best_kpt_dir, exist_ok=True)
    key_name = None
    for key in val_error.keys():
        if 'abs_rel' in key:
    ...
    # abs_rel = val_error['WildUAV_eval/abs_rel']
    abs_rel = val_error[key_name]
    best_save_name = os.path.join(best_kpt_dir, f'step%08d_{abs_rel:.4f}.pth' % curr_iter)
    best_saved_ckpts = glob.glob(best_kpt_dir + '/step*.pth')
    if len(best_saved_ckpts) == 0:
        torch.save(ckpt, best_save_name)
        logger.info(f'Save best depth model: {best_save_name}')
    else:
        pre_error = float(best_saved_ckpts[0].split('_')[-1].split('.pth')[0])
        if round(abs_rel, 4) < pre_error:
            os.remove(best_saved_ckpts.pop(0))
            torch.save(ckpt, best_save_name)
            logger.info(f'Save best depth model: {best_save_name}, Pre best removed')
    ...
    # # abs_rel = val_error['WildUAV_eval/abs_rel']
    # miou = seg_val_err[key_name]
    # best_safe_save_name = os.path.join(best_safe_kpt_dir, f'step%08d_{miou:.2f}.pth' % curr_iter)
    # best_safe_saved_ckpts = glob.glob(best_safe_kpt_dir + '/step*.pth')
    # if len(best_safe_saved_ckpts) == 0:
    #     torch.save(ckpt, best_safe_save_name)
    #     logger.info(f'Save best safe model: {best_safe_save_name}')
    # else:
    #     pre_miou = float(best_safe_saved_ckpts[0].split('_')[-1].split('.pth')[0])
    #     if round(miou, 2) > pre_miou:
    #         os.remove(best_safe_saved_ckpts.pop(0))
    #         torch.save(ckpt, best_safe_save_name)
    #         logger.info(f'Save best safe model: {best_safe_save_name}, Pre best removed')
