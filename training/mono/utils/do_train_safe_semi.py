...
def do_train_safe_semi(local_rank: int, cfg: dict):
    ...
    try:
        ...
        while True:
            ...
            with torch.cuda.amp.autocast(dtype=torch.float16):
                # pred_depth, losses_dict, conf = model(data)
                stu_output = model.module.forward_out(data)
            stu_output.update(data)
            labeled_dict, unlabeled_dict = split_data_by_label(stu_output)
            losses_dict = model.module.get_loss(labeled_dict)
            ...
            if cur_iter > semi_start_iter:
                ...
                with torch.cuda.amp.autocast(dtype=torch.float16):
                    teacher_output = teacher_model.module.module.forward_out(unlabel_data)
                # conf_thresh = 0.8
                ...
                depth_pseudo_target = torch.where(depth_mask,
                                                  ...
                                                  depth_pseudo_target)
                unlabeled_dict['target'] = depth_pseudo_target
                ...
                if cfg.with_safe:
                    ...
                    unlabeled_dict['safe_target'] = safe_pseudo_target

                unlabel_loss_dict = model.module.get_loss(unlabeled_dict)
